...
#define K5           A5                      // на реле K5  внешнее под различные нужды, програмно не реализован
...
if (/*VoltRead() < Vstart &&*/ digitalRead(Feedback_Pin) == LOW)  // Если напряжение АКБ меньше напряжения работающего двигателя, зажигание выключено то... 
...
//else if (at.indexOf("ALREADY CONNECT") > -1)     {SIM800.println("AT+CIPSEND"), delay (200); 
  ...
  else if (pin.indexOf("777") > -1 ){ pin= "", SIM800.println("AT+SAPBR=1,1");}           // костыль 3
