//        ТЕСТ.таймер 1 к 6             https://github.com/martinhol221/SIM800L_MQTT
...
#define Unlock_Pin  11                      // на реле вебасто или подогрева седушек
...
#define Lock_Pin     10                     // на дополнительное реле
#define RESET_Pin    5
...
int Timer, Security, inDS, count = 0;
...
void setup() {
 // pinMode(RESET_Pin, OUTPUT);             // указываем пин на выход для перезагрузки модема
  ...
  pinMode(Lock_Pin,    OUTPUT);             // указываем пин на выход для доп нужд.
  pinMode(Unlock_Pin,  OUTPUT);             // указываем пин на выход для доп реле вебасто
...
//  Serial.setTimeout(50);
 ...
 // SIM800.setTimeout(500);                 // тайм аут ожидания ответа
  ...
  Serial.println("MQTT |02/02/2018"); 
  delay (1000);
  SIM800_reset();
...
void heatingstop() {                                // программа остановки прогрева двигателя
    ...
    digitalWrite(Unlock_Pin, LOW), delay (100);
...
void MQTT_CONNECT () {
  ...
  SIM800.write(0x03), SIM800.write(0xC2),SIM800.write((byte)0),SIM800.write(0x3C); 
  ...
  MQTT_SUB ("C5/settimer");                                                        // пакет подписки
...
void  MQTT_SUB (const char MQTT_topic[15]) {                                       // пакет подписки на топик
  ...
  SIM800.write((byte)0), SIM800.write(0x01), SIM800.write((byte)0);                // 0x00
...
/*  -------------------------------------- проверяем соеденеиние с ИНТЕРНЕТ, конектимся к серверу------------------------------------------------------- */
   ...
   } else if (at.indexOf("C5/comandlock",4) > -1 )       {Security = at.substring(at.indexOf("")+17, at.indexOf("")+18).toInt();
   } else if (at.indexOf("C5/settimer",4) > -1 )         {Timer = at.substring(at.indexOf("")+15, at.indexOf("")+18).toInt();
   } else if (at.indexOf("C5/comandstop",4) > -1 )       { heatingstop();
   } else if (at.indexOf("C5/comandstart",4) > -1 )      {enginestart(2);
